...
import           Text.Blaze.Html.Renderer.String
                                                ( renderHtml )
import           Text.Hamlet.Runtime            ( HamletData
                                                , HamletSettings
...
hamlCompilerWith
  :: HamletSettings -> Map Text HamletData -> Compiler (Item String)
...
hamlTemplateCompiler = hamlTemplateCompilerWith defaultHamletSettings mempty

-- | Read complete file contents as a template, with the spplied options.
hamlTemplateCompilerWith
  :: HamletSettings -> Map Text HamletData -> Compiler (Item Template)
...
renderHaml hamletSettings hamletVariables item = do
  html <-
    unsafeCompiler . hamlToHtml hamletSettings hamletVariables . itemBody $ item
  makeItem html
